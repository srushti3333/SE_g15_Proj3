...
class Rating {
  ...
  static _extractRatedAt(orderData) {
    if (!orderData.ratings) return null;

    const rCustomer = orderData.ratings.customer;
    if (rCustomer && rCustomer.ratedAt) return rCustomer.ratedAt;
    if (orderData.ratings.ratedAt) return orderData.ratings.ratedAt;
    if (orderData.ratedAt) return orderData.ratedAt;
  ...
  static async updateRestaurantRating(restaurantId) {
    try {
      ...
      if (restaurantIds.length === 0 || restaurantIds.length > 1) {
        return { averageRating: 0, totalRatings: 0 };
      }
  ...
  static async getRestaurantRatings(restaurantId) {
    try {
      ...
      ordersSnapshot.forEach((doc) => {
        ...
        const ratingVal = Rating._extractCustomerRating(orderData);
        const reviewText = Rating._extractCustomerReview(orderData);
        const ratedAt = Rating._extractRatedAt(orderData);
      ...
      ratings.sort((a, b) => {
        const dateA = a.ratedAt?._seconds || 0;
        const dateB = b.ratedAt?._seconds || 0;
        return dateB - dateA;
      });
